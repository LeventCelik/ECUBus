...
#if (defined EN_DEBUG_FLS_MODULE) || (defined EN_UDS_DEBUG) || (defined EN_TP_DEBUG) || (defined EN_APP_DEBUG) ||      \
	(defined EN_DEBUG_FIFO)
...
// Security parameter; AES length, challenge size, etc.
#define SEC_PARAM			   (16u)
#define MAX_POWN_LEN		   (4096u) // XXX: Two bytes field, not sure if this is correct.
#define MAX_CERT_LEN		   (4096u) // XXX: Two bytes field, not sure if this is correct.
#define MAX_ADDTNL_PARAM_LEN   (4096u) // XXX: Two bytes field, not sure if this is correct.
#define EPHEMERAL_PUB_KEY_LEN  (2048u) // TODO: Set after crypto adoption.
#define EPHEMERAL_PRIV_KEY_LEN (2048u) // TODO: Set after crypto adoption.
#define SESSION_KEY_INFO_LEN   (256u)  // TODO: Correct this as needed.
...
#ifdef EN_CAN_TP
#define RX_FUN_ID  (0x7FFu) /* Broadcast client ID */
#define RX_PHY_ID  (0x784u) /* Supported client ID */
#define TX_ID	   (0x7F0u) /*can tp tx ID*/
#define RX_ID_MASK (0x700u) /*CAN TP rx mask*/
/*Enable TX CAN FD or not. If enable CAN FD, CAN TP transmit SF message will
 * over 8 Bytes*/
//#define EN_TX_CAN_FD
#endif
...
#ifdef EN_LIN_TP
#define RX_BOARD_ID                                                                                                    \
	(0x7Fu) /*LIN TP rx board ID -- all messages, response unexpected,                                                 \
			   but supported*/
#define RX_FUN_ID                                                                                                      \
	(0x7Eu)				  /*LIN tp rx function ID -- don't need  response/only                                         \
							 support SF*/
