...
static const flash_user_config_t Flash_InitConfig0 = {
	...
	/* If using callback, any code reachable from this function must not be
	 placed in a Flash block targeted for a program/erase operation.*/
	.CallBack = NULL_CALLBACK};

/* Declare a FLASH config struct which initialized by FlashInit, and will be
...
/* Declare a FLASH API struct which initialized by FlashInit, and will be used
 * by all flash operations */
const tFlashOptInfo *g_pstFlashOptInfo = &g_stFlashOptInfo;
#else
//#pragma CODE_SEG DEFAULT
/* Declare a Flash config struct which initialized by FLASH_DRV_Init, and will
 * be used all flash operation */
 ...
 *
 *END**************************************************************************/
static void FLASH_DRV_GetDEPartitionCode(flash_ssd_config_t *const pSSDConfig, uint8_t DEPartitionCode) {
	/* Select D-Flash size */
 ...
 *END**************************************************************************/
static status_t FLASH_DRV_Init(const flash_user_config_t *const pUserConf, flash_ssd_config_t *const pSSDConfig) {
...
void InitFlashAPI(void) {
	...
	/* Disable cache to ensure that all flash operations will take effect
	 instantly, this is device dependent */
...
/*erase flash*/
unsigned char EraseFlashSector(const unsigned long i_ulLogicalAddr, const unsigned long i_ulEraseLen) {
...
unsigned char WriteFlash(const uint32_t i_xStartAddr, const void *i_pvDataBuf, const unsigned short i_usDataLen) {
	uint32_t failAddr;
	status_t ret; /* Store the driver APIs return code */
...
/*flash erase verify*/
unsigned char FlashEraseVerify(const unsigned long i_ulStartVerifyAddr, const unsigned long i_ulVerifyLen) {
	status_t ret; /* Store the driver APIs return code */
 ...
 *********************************************************/
void ReadFlashMemory(const unsigned long i_ulLogicalAddr, const unsigned long i_ulLength, unsigned char *o_pucDataBuf) {
