...
typedef struct {
	...
	uint32_t DFlashBase;	   /*!< For FlexNVM device, this is the base address of
								* D-Flash memory (FlexNVM memory); For non-FlexNVM
	...
	flash_callback_t CallBack; /*!< Call back function to service the time
								* critical events. Any code reachable from this
								* function must not be placed in a Flash block
...
typedef struct {
	...
	uint32_t DFlashBase;	   /*!< For FlexNVM device, this is the base address of
								* D-Flash memory (FlexNVM memory); For non-FlexNVM
								* device, this field is unused */
	uint32_t DFlashSize;	   /*!< For FlexNVM device, this is the size in byte of
								* area which is used as D-Flash from FlexNVM memory;
								*    For non-FlexNVM device, this field is unused */
	uint32_t EERAMBase;		   /*!< The base address of FlexRAM (for FlexNVM device)
								*    or acceleration RAM memory (for non-FlexNVM device) */
	uint32_t EEESize;		   /*!< For FlexNVM device, this is the size in byte of
								* EEPROM area which was partitioned from FlexRAM; For
...
typedef struct {
	...
	status_t (*FLASH_VerifySection)(const flash_ssd_config_t *pSSDConfig, uint32_t dest, uint16_t number,
									uint8_t marginLevel);
	...
	status_t (*FLASH_ProgramCheck)(const flash_ssd_config_t *pSSDConfig, uint32_t dest, uint32_t size,
...
extern unsigned char EraseFlashSector(const unsigned long i_ulLogicalAddr, const unsigned long i_ulEraseLen);

extern unsigned char WriteFlash(const uint32_t i_xStartAddr, const void *i_pvDataBuf, const unsigned short i_usDataLen);

/*flash erase verify*/
extern unsigned char FlashEraseVerify(const unsigned long i_ulStartVerifyAddr, const unsigned long i_ulVerifyLen);
 ...
 *********************************************************/
extern void ReadFlashMemory(const unsigned long i_ulLogicalAddr, const unsigned long i_ulLength,
