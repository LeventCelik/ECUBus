...
typedef enum { TX_MSG_SUCCESSFUL = 0u, TX_MSG_FAILD, TX_MSG_TIMEOUT } tTxMsgStatus;
...
extern tUdsId TP_GetConfigTxMsgID(void);

/*Get TP config recieve Funcation ID*/
extern tUdsId TP_GetConfigRxMsgFUNID(void);

/*Get TP config receive pyhiscal ID*/
extern tUdsId TP_GetConfigRxMsgPHYID(void);
...
extern boolean TP_DriverReadDataFromTP(const uint32 i_readDataLen, uint8 *o_pReadDatabuf, uint32 *o_pTxMsgID,
									   uint32 *o_pTxMsgLength);
