...
#define IsReciveCFAll(xCFDataLen)                                                                                      \
	(((gs_stLINTPRxDataInfo.stLINTpDataInfo.xPduDataLen + (uint8)(xCFDataLen)) >=                                      \
...
#define IsRxBlockSizeOverflow()                                                                                        \
	(((0u != g_stUdsLINNetLayerCfgInfo.xBlockSize) &&                                                                  \
...
/*Is Tx all*/
#define IsTxAll()                                                                                                      \
	((gs_stLINTPTxDataInfo.stLINTpDataInfo.xPduDataLen >= gs_stLINTPTxDataInfo.stLINTpDataInfo.xFFDataLen) ? TRUE      \
																										   : FALSE)
...
static uint8 LINTP_SetFrameType(const tNetWorkFrameType i_eFrameType, uint8 *o_pucFrameType);

/*received a can tp frame, copy these data in fifo.*/
static uint8 LINTP_CopyAFrameDataInRxFifo(const tUdsId i_xRxCanID, const tLen i_xRxDataLen, const uint8 *i_pucDataBuf);

/*uds transmitted a application frame data, copy these data in TX fifo.*/
static uint8 LINTP_CopyAFrameFromFifoToBuf(tUdsId *o_pxTxCanID, uint8 *o_pucTxDataLen, uint8 *o_pucDataBuf);
...
const static tLINTpFunInfo gs_astLINTpFunInfo[] = {
	{LINTP_IDLE, LINTP_DoLINTPIdle},   {LINTP_RX_SF, LINTP_DoReceiveSF},
	{LINTP_RX_FF, LINTP_DoReceiveFF},  {LINTP_RX_CF, LINTP_DoReceiveCF},

	{LINTP_TX_SF, LINTP_DoTransmitSF}, {LINTP_TX_FF, LINTP_DoTransmitFF},
	{LINTP_TX_CF, LINTP_DoTransmitCF}, {LINTP_WAITTING_TX, LINTP_DoWaittingTxMsg}};
...
void LINTP_Init(void) {
	...
	if (ERRO_NONE != eStatus) {
		TP_DebugPrintf("apply fifo RX erro!\n");
		while (1) {
		}
	}
	...
	if (ERRO_NONE != eStatus) {
#ifdef EN_TP_DEBUG
		TP_DebugPrintf("apply fifo TX erro code!\n");
#endif
		while (1) {
		}
	}
	...
	if (ERRO_NONE != eStatus) {
		TP_DebugPrintf("apply RX fifo from BUS erro code!\n");
		while (1) {
		}
	}
	...
	if (ERRO_NONE != eStatus) {
		TP_DebugPrintf("apply TX fifo TO BUS erro code!\n");
		while (1) {
		}
	}
#endif
...
void LINTP_MainFun(void) {
	...
	if (LINTP_WAITTING_TX != GetCurLINTpStatus()) {
		...
		if (TRUE ==
			g_stUdsLINNetLayerCfgInfo.pfNetRx(&stRxLINTpMsg.xMsgId, &stRxLINTpMsg.msgLen, stRxLINTpMsg.aMsgBuf)) {
			/*check received message ID*/
			if (TRUE == LINTP_IsReceivedMsgIDValid(stRxLINTpMsg.xMsgId)) {
				stRxLINTpMsg.isFree = FALSE;

			} else {
				TP_DebugPrintf("recevied invalid message ID\n");
			}
		}
	}
	...
	while (index < findCnt) {
		...
		if (N_UNEXP_PDU != result) {
			...
			/*if received unexpect PDU, then jump to LINTP_IDLE and restart do
			 * process.*/
			index = 0u;
		}
	}
...
static uint8 LINTP_CopyAFrameFromFifoToBuf(tUdsId *o_pxTxCanID, uint8 *o_pucTxDataLen, uint8 *o_pucDataBuf) {
	...
	/*read data from fifo*/
	ReadDataFromFifo(TX_TP_QUEUE_ID, exchangeMsgInfo.dataLen, o_pucDataBuf, &xRealReadLen, &eStatus);
...
static tN_Result LINTP_DoLINTPIdle(tLINTpMsg *m_stMsgInfo, tLINTpWorkStatus *m_peNextStatus) {
	...
	if (FALSE == m_stMsgInfo->isFree) {
		...
		if (TRUE == LINTP_CopyAFrameFromFifoToBuf(&gs_stLINTPTxDataInfo.stLINTpDataInfo.xLINTpId, &TxDataLen,
...
static tN_Result LINTP_DoReceiveSF(tLINTpMsg *m_stMsgInfo, tLINTpWorkStatus *m_peNextStatus) {
	...
	if (FALSE == LINTP_CopyAFrameDataInRxFifo(m_stMsgInfo->xMsgId, SFLen, &m_stMsgInfo->aMsgBuf[1u])) {
...
static tN_Result LINTP_DoReceiveFF(tLINTpMsg *m_stMsgInfo, tLINTpWorkStatus *m_peNextStatus) {
	...
	if (FFDataLen < FF_DATA_MIN_LEN) {
		TP_DebugPrintf("Received not FF data len less than min.\n");

#ifdef EN_TP_DEBUG
		TP_DebugPrintf("Received FF data len = %d\n", FFDataLen);
#endif
	...
	fsl_memcpy(gs_stLINTPRxDataInfo.stLINTpDataInfo.aDataBuf, (const void *)&m_stMsgInfo->aMsgBuf[2u],
...
static tN_Result LINTP_DoTransmitSF(tLINTpMsg *m_stMsgInfo, tLINTpWorkStatus *m_peNextStatus) {
	...
	/*copy data in tx buf*/
	fsl_memcpy(&aDataBuf[1u], gs_stLINTPTxDataInfo.stLINTpDataInfo.aDataBuf,
	...
	if (TRUE != g_stUdsLINNetLayerCfgInfo.pfNetTxMsg(gs_stLINTPTxDataInfo.stLINTpDataInfo.xLINTpId, TxLen, aDataBuf,
...
static tN_Result LINTP_DoTransmitFF(tLINTpMsg *m_stMsgInfo, tLINTpWorkStatus *m_peNextStatus) {
	...
	/*request transmitted application message.*/
	if (TRUE != g_stUdsLINNetLayerCfgInfo.pfNetTxMsg(gs_stLINTPTxDataInfo.stLINTpDataInfo.xLINTpId, sizeof(aDataBuf),
													 aDataBuf, LINTP_TxMsgSuccessfulCallBack,
													 g_stUdsLINNetLayerCfgInfo.txBlockingMaxTimeMs)) {
		/*set tx message status is waitting status and register callback*/
		LINTP_SetTxMsgStatus(LINTP_TX_MSG_FAIL);
		LINTP_RegisterTxMsgCallBack(NULL_PTR);
...
static tN_Result LINTP_DoTransmitCF(tLINTpMsg *m_stMsgInfo, tLINTpWorkStatus *m_peNextStatus) {
	...
	if (TxLen >= (CF_DATA_MAX_LEN)) {
		...
		if (TRUE != g_stUdsLINNetLayerCfgInfo.pfNetTxMsg(gs_stLINTPTxDataInfo.stLINTpDataInfo.xLINTpId,
														 sizeof(aTxDataBuf), aTxDataBuf, LINTP_TxMsgSuccessfulCallBack,
														 g_stUdsLINNetLayerCfgInfo.txBlockingMaxTimeMs)) {
			/*set tx message status is waitting status and register callback*/
			LINTP_SetTxMsgStatus(LINTP_TX_MSG_FAIL);
			LINTP_RegisterTxMsgCallBack(NULL_PTR);
		...
		/*request transmitted application message.*/
		if (TRUE != g_stUdsLINNetLayerCfgInfo.pfNetTxMsg(gs_stLINTPTxDataInfo.stLINTpDataInfo.xLINTpId, aTxAllLen,
														 aTxDataBuf, LINTP_TxMsgSuccessfulCallBack,
														 g_stUdsLINNetLayerCfgInfo.txBlockingMaxTimeMs)) {
			/*set tx message status is waitting status and register callback*/
			LINTP_SetTxMsgStatus(LINTP_TX_MSG_FAIL);
			LINTP_RegisterTxMsgCallBack(NULL_PTR);
...
static uint8 LINTP_SetFrameType(const tNetWorkFrameType i_eFrameType, uint8 *o_pucFrameType) {
	...
	if (SF == i_eFrameType || FF == i_eFrameType || CF == i_eFrameType) {
...
/*LIN TP TX message callback*/
static void LINTP_TxMsgSuccessfulCallBack(void) { gs_eLINTPTxMsStatus = LINTP_TX_MSG_SUCC; }

/*LIN TP set TX message status*/
static void LINTP_SetTxMsgStatus(const tLINTPTxMsgStatus i_eTxMsgStatus) { gs_eLINTPTxMsStatus = i_eTxMsgStatus; }
