...
static uint8 LINTP_TxMsg(const tUdsId i_xTxId, const uint16 i_DataLen, const uint8 *i_pDataBuf,
						 const tpfNetTxCallBack i_pfNetTxCallBack, const uint32 txBlockingMaxtime);

static uint8 LINTP_RxMsg(tUdsId *o_pxRxId, uint8 *o_pRxDataLen, uint8 *o_pRxBuf);
...
/*LIN tp tx message: there not use CAN driver TxFIFO, directly invoked CAN send
 * function*/
static uint8 LINTP_TxMsg(const tUdsId i_xTxId, const uint16 i_DataLen, const uint8 *i_pDataBuf,
						 const tpfNetTxCallBack i_pfNetTxCallBack, const uint32 txBlockingMaxtime) {
...
boolean LINTP_DriverReadDataFromLINTP(const uint32 i_readDataLen, uint8 *o_pReadDataBuf,
...
tUdsId LINTP_GetConfigTxMsgID(void) { return g_stUdsLINNetLayerCfgInfo.xTxId; }

/*get config LIN TP recevie function message ID*/
tUdsId LINTP_GetConfigRxMsgFUNID(void) { return g_stUdsLINNetLayerCfgInfo.xRxFunId; }

/*get config LIN TP recevie physical message ID*/
tUdsId LINTP_GetConfigRxMsgPHYID(void) { return g_stUdsLINNetLayerCfgInfo.xRxPhyId; }

/*get config LIN TP recevie broadcast message ID*/
tUdsId LINTP_GetConfigRxMsgBroadcastID(void) { return g_stUdsLINNetLayerCfgInfo.xRxBoardcastId; }

/*Get LIN TP config Tx  handle*/
tNetTxMsg LINTP_GetConfigTxHandle(void) { return g_stUdsLINNetLayerCfgInfo.pfNetTxMsg; }

/*Get LIN TP config Rx  handle*/
tNetRx LINTP_GetConfigRxHandle(void) { return g_stUdsLINNetLayerCfgInfo.pfNetRx; }

/*Is received message valid?*/
boolean LINTP_IsReceivedMsgIDValid(const uint32 i_receiveMsgID) {
	boolean result = FALSE;

	if ((i_receiveMsgID == LINTP_GetConfigRxMsgFUNID()) || (i_receiveMsgID == LINTP_GetConfigRxMsgPHYID()) ||
...
static void LINTP_AbortTxMsg(void) {
	...
	if (TRUE != LINTP_ClearTXBUSFIFO()) {
		TP_DebugPrintf("AbortTxMsg: Clear TX BUS FIFO failed!\n");
	}
}
