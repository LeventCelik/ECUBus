...
typedef enum {
	N_OK = 0,		/*This value means that the service execution has completed
		   successfully; it can be issued to a service user on both the
		   sender and receiver side*/
	N_TIMEOUT_A,	/*This value is issued to the protocol user when the timer
		N_Ar/N_As has passed its time-out
		value N_Asmax/N_Armax; it can be issued to service
		user on both the sender and receiver side.*/
	N_TIMEOUT_Bs,	/*This value is issued to the service user when the timer N_Bs
	   has passed its time-out value
	   N_Bsmax; it can be issued to the service user on
	   the sender side only.*/
	N_TIMEOUT_Cr,	/*This value is issued to the service user when the timer N_Cr
	   has passed its time-out value N_Crmax; it can be issued to
	   the service user on the receiver side only.*/
	N_WRONG_SN,		/*This value is issued to the service user upon reception of an
		 unexpected sequence number
		 (PCI.SN) value; it can be issued to the service user on the
		 receiver side only.*/
	N_INVALID_FS,	/*This value is issued to the service user when an invalid or
	   unknown FlowStatus value has
	   been received in a flow control (FC) N_PDU; it can be issued
	   to the service user on the sender side only.*/
	N_UNEXP_PDU,	/*This value is issued to the service user upon reception of an
		unexpected protocol data unit; it can be issued to the
		service user on the receiver side only.*/
	N_WTF_OVRN,		/*This value is issued to the service user upon reception of
		 flow control WAIT frame that  exceeds the maximum counter
		 N_WFTmax.*/
	N_BUFFER_OVFLW, /*This value is issued to the service user upon reception of
	 a flow control (FC) N_PDU with FlowStatus = OVFLW. It
	 ...
	 parameter in the FirstFrame and therefore the transmission
	 of the segmented message was aborted. It can be issued to
	 the service user on the sender side only.*/
	N_ERROR			/*This is the general error value. It shall be issued to the service
			 user when an error has been detected by the network layer and no
			 other parameter value can be used to better describe
			 the error. It can be issued to the service user on both the sender
			 and receiver side.*/
} tN_Result;
...
#define IsRxMsgLenValid(address_type, frameLen, RXCANMsgLen)                                                           \
	((address_type == NORMAL_ADDRESSING) ? (frameLen <= RXCANMsgLen - 1) : (frameLen <= RXCANMsgLen - 2))
...
#define IsReciveCFAll(xCFDataLen)                                                                                      \
	(((gs_stCanTPRxDataInfo.stCanTpDataInfo.xPduDataLen + (uint8)(xCFDataLen)) >=                                      \
...
#define IsRxBlockSizeOverflow()                                                                                        \
	(((0u != g_stCANUdsNetLayerCfgInfo.xBlockSize) &&                                                                  \
...
/*clear Tx data len*/
#define ClearTxDataLen() (gs_stCanTPTxDataInfo.stCanTpDataInfo.xPduDataLen = 0u)
...
/*Is Tx all*/
#define IsTxAll()                                                                                                      \
	((gs_stCanTPTxDataInfo.stCanTpDataInfo.xPduDataLen >= gs_stCanTPTxDataInfo.stCanTpDataInfo.xFFDataLen) ? true      \
																										   : false)
...
static boolean CANTP_FillTxMsgInfo(const tNetWorkFrameType i_eFrameType, const uint32 i_srcMsgLen,
								   const uint8 *i_pSrcMsgBuf, const uint32 i_maxDstMsgBufLen, uint8 *o_pDstMsgBuf,
								   uint32 *o_pDstMsgLen);

/*received a can tp frame, copy these data in fifo.*/
static uint8 CANTP_CopyAFrameDataInRxFifo(const tUdsId i_xRxCanID, const tLen i_xRxDataLen, const uint8 *i_pDataBuf);
...
const static tCanTpFunInfo gs_astCanTpFunInfo[] = {
	{IDLE, CANTP_DoCanTpIdle},	 {RX_SF, CANTP_DoReceiveSF},		  {RX_FF, CANTP_DoReceiveFF},
	{TX_FC, CANTP_DoTransmitFC}, {RX_CF, CANTP_DoReceiveCF},

	{TX_SF, CANTP_DoTransmitSF}, {TX_FF, CANTP_DoTransmitFF},		  {RX_FC, CANTP_DoReceiveFC},
	{TX_CF, CANTP_DoTransmitCF}, {WAITTING_TX, CANTP_DoWaittingTxMsg}};
...
void CANTP_Init(void) {
	...
	if (ERRO_NONE != eStatus) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "apply RX_TP_QUEUE_ID failed!\n");
		while (1) {
		}
	}
	...
	if (ERRO_NONE != eStatus) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "apply TX_TP_QUEUE_ID failed\n");
		while (1) {
		}
	}
	...
	if (ERRO_NONE != eStatus) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "apply RX_BUS_FIFO failed!\n");
		while (1) {
		}
	}
	...
	if (ERRO_NONE != eStatus) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "apply TX_BUS_FIFOfailed!\n");
		while (1) {
		}
	}
}
...
void CANTP_MainFun(void) {
	...
	/*In waitting TX message, cannot read message from FIFO. Because, In
	 * waitting message will lost read messages.*/
	if (WAITTING_TX != GetCurCANTPStatus()) {
		/*read msg from CAN driver RxFIFO*/
		if (true ==
			g_stCANUdsNetLayerCfgInfo.pfNetRx(&stRxCanTpMsg.xMsgId, &stRxCanTpMsg.msgLen, stRxCanTpMsg.aMsgBuf)) {
	...
	while (index < findCnt) {
		...
		/*if received unexpect PDU, then jump to IDLE and restart do
		 * progrocess.*/
...
static uint8 CANTP_CopyAFrameFromFifoToBuf(tUdsId *o_pxTxCanID, uint8 *o_pTxDataLen, uint8 *o_pDataBuf) {
	...
	/*read receive ID*/
	ReadDataFromFifo(TX_TP_QUEUE_ID, sizeof(tUDSAndTPExchangeMsgInfo), (uint8 *)&exchangeMsgInfo, &xRealReadLen,
					 &eStatus);
	...
	/*read data from fifo*/
	ReadDataFromFifo(TX_TP_QUEUE_ID, exchangeMsgInfo.dataLen, o_pDataBuf, &xRealReadLen, &eStatus);
	...
	if (false == m_stMsgInfo->isFree) {
		if (true == IsSF(m_stMsgInfo->aMsgBuf[0u])) {
			...
			*m_peNextStatus = RX_FF;
			...
			TP_DebugPrintf(LOG_LEVEL_ERROR, "%s received invalid message!\n", __func__);
		}
	} else {
		/*Judge have message can will tx.*/
		if (true == CANTP_CopyAFrameFromFifoToBuf(&gs_stCanTPTxDataInfo.stCanTpDataInfo.xCanTpId, &txDataLen,
...
static tN_Result CANTP_DoReceiveSF(tCanTpMsg *m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus) {
	...
	if ((0u == m_stMsgInfo->msgLen) || m_stMsgInfo->isFree) {
		return N_ERROR;
	}

	if (!IsSF(m_stMsgInfo->aMsgBuf[0u])) {
		return N_ERROR;
	}

	/*Get RX frame: SF length*/
	if (!GetRXSFFrameMsgLength(m_stMsgInfo->msgLen, m_stMsgInfo->aMsgBuf, &SFLen)) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "SF:GetRXSFFrameMsgLength failed!\n");
		return N_ERROR;
	}

	/*Get Rx message valid position*/
	if (!GetRxSFMsgValidPosition(m_stMsgInfo->msgLen, &dataStartPos)) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "SF: GetRxSFMsgValidPosition failed!\n");
		return N_ERROR;
	}

	/*write data to UDS fifo*/
	if (!CANTP_CopyAFrameDataInRxFifo(m_stMsgInfo->xMsgId, SFLen, &m_stMsgInfo->aMsgBuf[dataStartPos])) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "Copy data error!\n");
		return N_ERROR;
	}
...
static tN_Result CANTP_DoReceiveFF(tCanTpMsg *m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus) {
	...
	if (!IsFF(m_stMsgInfo->aMsgBuf[0u])) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "Received not FF\n");

		return N_ERROR;
	}

	/*get FF Data len*/
	if (!GetRXFFFrameMsgLength(m_stMsgInfo->msgLen, m_stMsgInfo->aMsgBuf, &FFDataLen)) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "FF:GetRXFrameMsgLength failed!\n");
		return N_ERROR;
	}
	...
	/*write data in global buf. When receive all data, write these data in
	 * fifo.*/
	...
	fsl_memcpy(gs_stCanTPRxDataInfo.stCanTpDataInfo.aDataBuf, (const void *)&m_stMsgInfo->aMsgBuf[2u],
			   m_stMsgInfo->msgLen - 2u);
...
static tN_Result CANTP_DoReceiveCF(tCanTpMsg *m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus) {
	...
	if (IsWaitCFTimeout()) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "Connective frame timeout!\n");
		*m_peNextStatus = IDLE;
		return N_TIMEOUT_Cr;
	}

	if (0u == m_stMsgInfo->msgLen || m_stMsgInfo->isFree) {
		/*waitting CF message, It's normally for not received CAN message in the
		 * step.*/
		return N_OK;
	}

	/*check received msssage is SF or FF? If received SF or FF, start new
	 * receive progrocess.*/
	if (IsSF(m_stMsgInfo->aMsgBuf[0u]) || IsFF(m_stMsgInfo->aMsgBuf[0u])) {
		TP_DebugPrintf(LOG_LEVEL_DEBUG, "In receive progrocess: received SF\n");
		*m_peNextStatus = IDLE;
		return N_UNEXP_PDU;
	}
...
#ifdef EN_TP_DEBUG
		TP_DebugPrintf(LOG_LEVEL_ERROR, "Msg ID invalid in CF! RX ID = %lX, RX ID = %lX\n",
	...
	if (!IsCF(m_stMsgInfo->aMsgBuf[0u])) {
#ifdef EN_TP_DEBUG
		TP_DebugPrintf(LOG_LEVEL_ERROR, "Msg type invalid in CF %X!\n", m_stMsgInfo->aMsgBuf[0u]);
#endif
	...
	if (!IsRevSNValid(m_stMsgInfo->aMsgBuf[0u])) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "Msg SN invalid(0x%X), expect SN(0x%X)in CF!\n", m_stMsgInfo->aMsgBuf[0u],
					   gs_stCanTPRxDataInfo.ucSN);
	...
	if (true == IsReciveCFAll(m_stMsgInfo->msgLen - 1u)) {
		...
		*m_peNextStatus = IDLE;
		...
		fsl_memcpy(&gs_stCanTPRxDataInfo.stCanTpDataInfo.aDataBuf[gs_stCanTPRxDataInfo.stCanTpDataInfo.xPduDataLen],
				   &m_stMsgInfo->aMsgBuf[1u], m_stMsgInfo->msgLen - 1u);
...
static tN_Result CANTP_DoTransmitFC(tCanTpMsg *m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus) {
	...
	/*Is wait FC timeout?*/
	if (!IsWaitFCTimeout()) {
		TP_DebugPrintf(LOG_LEVEL_DEBUG, "Transmit Flow Control has not timed out!\n");
	...
	/*CAN TP set tx message status and register tx message successful
	 * callback.*/
	...
	if (true == g_stCANUdsNetLayerCfgInfo.pfNetTxMsg(txMsgId, sizeof(aucTransDataBuf), aucTransDataBuf,
...
static tN_Result CANTP_DoTransmitSF(tCanTpMsg *m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus) {
	...
	if (true != CANTP_FillTxMsgInfo(SF, gs_stCanTPTxDataInfo.stCanTpDataInfo.xFFDataLen,
									gs_stCanTPTxDataInfo.stCanTpDataInfo.aDataBuf, DATA_LEN, aDataBuf, &txLen)) {
		return N_ERROR;
	}
	...
	if (true != g_stCANUdsNetLayerCfgInfo.pfNetTxMsg(gs_stCanTPTxDataInfo.stCanTpDataInfo.xCanTpId, txLen, aDataBuf,
													 CANTP_TxMsgSuccessfulCallBack,
													 g_stCANUdsNetLayerCfgInfo.txBlockingMaxTimeMs)) {
		/*CAN TP set tx message status and register tx message successful
		 * callback.*/
...
static tN_Result CANTP_DoTransmitFF(tCanTpMsg *m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus) {
	...
	/*Check transmit data len. If data len overflow less than SF, return
	 * FALSE.*/
	if (true != IsTxDataLenOverflowSF()) {
		*m_peNextStatus = TX_SF;
	...
	if (true != CANTP_FillTxMsgInfo(FF, gs_stCanTPTxDataInfo.stCanTpDataInfo.xFFDataLen,
									gs_stCanTPTxDataInfo.stCanTpDataInfo.aDataBuf, DATA_LEN, aDataBuf, &txMsgLen)) {
		return N_ERROR;
	}
#endif
	...
	/*request transmitted application message.*/
	if (true != g_stCANUdsNetLayerCfgInfo.pfNetTxMsg(gs_stCanTPTxDataInfo.stCanTpDataInfo.xCanTpId, txMsgLen, aDataBuf,
													 CANTP_TxMsgSuccessfulCallBack,
													 g_stCANUdsNetLayerCfgInfo.txBlockingMaxTimeMs)) {
		/*CAN TP set tx message status and register tx message successful
		 * callback.*/
...
static tN_Result CANTP_DoReceiveFC(tCanTpMsg *m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus) {
	...
	if (IsTxWaitFrameTimeout()) {
		TP_DebugPrintf(LOG_LEVEL_WARNING, "Wait flow control timeout.\n");
		*m_peNextStatus = IDLE;
		return N_TIMEOUT_Cr;
	}

	if ((0u == m_stMsgInfo->msgLen) || m_stMsgInfo->isFree) {
		/*waiting received FC. It's normally for waiting CAN message and
		 * return OK.*/
		return N_OK;
	}

	if (!IsFC(m_stMsgInfo->aMsgBuf[0u])) {
		return N_ERROR;
	}
	...
	if (CONTINUE_TO_SEND == eFlowStatus) {
		...
		/*remove Add TX SN, because this SN is added in send First frame
		 * callback*/
#if 0
		AddTxSN();
#endif
...
static void CANTP_DoTransmitCFCallBack(void) {
	...
	if (gs_stCanTPTxDataInfo.ucBlockSize) {
		...
		/*block size is equal 0,  waitting  flow control message. if not equal
		 * 0, continual send CF message.*/
...
static tN_Result CANTP_DoTransmitCF(tCanTpMsg *m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus) {
	...
	if (txLen >= TX_CF_DATA_MAX_LEN) {
		...
		if (true != g_stCANUdsNetLayerCfgInfo.pfNetTxMsg(gs_stCanTPTxDataInfo.stCanTpDataInfo.xCanTpId,
														 sizeof(aTxDataBuf), aTxDataBuf, CANTP_TxMsgSuccessfulCallBack,
														 g_stCANUdsNetLayerCfgInfo.txBlockingMaxTimeMs)) {
			/*CAN TP set tx message status and register tx message successful
			 * callback.*/
			CANTP_SetTxMsgStatus(CANTP_TX_MSG_FAIL);
		...
		/*request transmitted application message.*/
		if (true != g_stCANUdsNetLayerCfgInfo.pfNetTxMsg(gs_stCanTPTxDataInfo.stCanTpDataInfo.xCanTpId, txAllLen,
														 aTxDataBuf, CANTP_TxMsgSuccessfulCallBack,
														 g_stCANUdsNetLayerCfgInfo.txBlockingMaxTimeMs)) {
			/*CAN TP set tx message status and register tx message successful
			 * callback.*/
			CANTP_SetTxMsgStatus(CANTP_TX_MSG_FAIL);
...
static boolean CANTP_SetTxFFDataLen(const uint32 i_txSFDataLen, uint8 *o_pFFMsgBuf) {
	...
	if (i_txSFDataLen > 0x0FFFu) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "CAN TP TX message length over max!\n");
		return false;
	}
...
static boolean CANTP_FillTxMsgInfo(const tNetWorkFrameType i_eFrameType, const uint32 i_srcMsgLen,
	...
	if ((0 == i_srcMsgLen) || (NULL_PTR == i_pSrcMsgBuf) ||
...
/*CAN TP TX message callback*/
static void CANTP_TxMsgSuccessfulCallBack(void) { gs_eCANTPTxMsStatus = CANTP_TX_MSG_SUCC; }

/*CANP TP set TX message status*/
static void CANTP_SetTxMsgStatus(const tCanTPTxMsgStatus i_eTxMsgStatus) { gs_eCANTPTxMsStatus = i_eTxMsgStatus; }
...
static void CANTP_DoRegisterTxMsgCallBack(void) {
	...
	/*get the tx message status with disable interrupt for protect the variable
	 * not changeb by interrupt.*/
	...
	if (CANTP_TX_MSG_SUCC == CANTPTxMsgStatus) {
		...
		TP_DebugPrintf(LOG_LEVEL_ERROR, "TX message failed callback=%p, status=%d\n", gs_pfCANTPTxMsgCallBack, gs_eCANTPTxMsStatus);
