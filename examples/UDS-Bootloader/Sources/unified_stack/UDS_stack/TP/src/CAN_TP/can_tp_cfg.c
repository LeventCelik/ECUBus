...
static uint8 CANTP_TxMsg(const tUdsId i_xTxId, const uint16 i_dataLen, const uint8 *i_pDataBuf,
						 const tpfNetTxCallBack i_pfNetTxCallBack, const uint32 i_txBlockingMaxtime);
...
static boolean CANTP_FillPaddingData(const uint32 i_maxMsgLen, const uint32 i_msgLen, uint8 *m_pMsgBuf,
									 uint32 *o_pTxMsgLen);
...
const tUdsCANNetLayerCfg g_stCANUdsNetLayerCfgInfo = {
	...
	900u,		 /*N_Cs < 0.9 N_Cr*/
	1000u,		 /*N_Cr*/
	0u,			 /*max blocking time 0ms, > 0u mean waitting send successful. equal 0 is
					not waitting.*/
...
/*can tp tx message: there not use CAN driver TxFIFO, directly invoked CAN send
 * function*/
static uint8 CANTP_TxMsg(const tUdsId i_xTxId, const uint16 i_dataLen, const uint8 *i_pDataBuf,
						 const tpfNetTxCallBack i_pfNetTxCallBack, const uint32 i_txBlockingMaxtime) {
...
static uint8 CANTP_RxMsg(tUdsId *o_pxRxId, uint8 *o_pRxDataLen, uint8 *o_pRxBuf) {
	...
	if ((ERRO_NONE == eStatus) && (headerLen <= xCanRxDataLen)) {
		ReadDataFromFifo(RX_BUS_FIFO, headerLen, (uint8 *)&stRxCanMsg, &xReadDataLen, &eStatus);

		if ((ERRO_NONE == eStatus) && (headerLen <= xCanRxDataLen)) {
			ReadDataFromFifo(RX_BUS_FIFO, stRxCanMsg.rxDataLen, (uint8 *)&stRxCanMsg.aucDataBuf, &xCanRxDataLen,
							 &eStatus);
		...
		if ((0u != xCanRxDataLen) || (ERRO_NONE != eStatus)) {
			TP_DebugPrintf(LOG_LEVEL_ERROR, "%s write message in FIFO failed! status = %d, "
						   "FIFO avaliable Length=%d\n", __func__, eStatus, xCanRxDataLen);
		}
	}
...
tUdsId CANTP_GetConfigTxMsgID(void) { return g_stCANUdsNetLayerCfgInfo.xTxId; }

/*get config CAN TP recevie function message ID*/
tUdsId CANTP_GetConfigRxMsgFUNID(void) { return g_stCANUdsNetLayerCfgInfo.xRxFunId; }
...
tUdsId CANTP_GetConfigRxMsgPHYID(void) { return g_stCANUdsNetLayerCfgInfo.xRxPhyId; }

/*Get CAN TP config Tx  handle*/
tNetTxMsg CANTP_GetConfigTxHandle(void) { return g_stCANUdsNetLayerCfgInfo.pfNetTxMsg; }

/*Get CAN TP config Rx  handle*/
tNetRx CANTP_GetConfigRxHandle(void) { return g_stCANUdsNetLayerCfgInfo.pfNetRx; }

/*abort CAN BUS tx message*/
static void CANTP_AbortTxMsg(void) {
	TP_DebugPrintf(LOG_LEVEL_INFO, "CANTP_AbortTxMsg\n");

	if (gs_pfCANTPAbortTxMsg != NULL_PTR) {
		(gs_pfCANTPAbortTxMsg)();

		gs_pfTxMsgSuccessfulCallBack = NULL_PTR;
	}

	if (!CANTP_ClearTXBUSFIFO()) {
		TP_DebugPrintf(LOG_LEVEL_ERROR, "CANTP_AbortTxMsg: Clear TX BUS FIFO failed!\n");
	}
}
...
boolean CANTP_DriverReadDataFromCANTP(const uint32 i_readDataLen, uint8 *o_pReadDataBuf,
		...
		if (true == result) {
			...
			if ((ERRO_NONE == eStatus) && (i_readDataLen >= txMsgInfo.TxMsgLength)) {
				...
				*o_pstTxMsgHeader = txMsgInfo;
				...
				/*storage callback, if user want to TX message callback please
				 * call TP_DoTxMsgSuccesfulCallback or self call callback*/
	...
	if ((true == res)) {
		if (i_maxMsgLen >= txMsgLen) {
			*o_pTxMsgLen = txMsgLen;
			...
			TP_DebugPrintf(LOG_LEVEL_WARNING, "CANTP_FillPaddingData, tx message length=%lX is not right!\n", txMsgLen);
		}
	}
