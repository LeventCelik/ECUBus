...
typedef struct {
	/*flash programming successfull? If programming successfull, the value set
	 * TRUE, else set FALSE*/
	bool isFlashProgramSuccessfull;

	/*Is erase flash successfull? If erased flash successfull, set the TRUE,
	 * else set the FALSE.*/
	bool isFlashErasedSuccessfull;

	/*Is Flash struct data valid? If writen set the value is TRUE, else set the
	 * valid FALSE*/
	bool isFlashStructValid;
...
#define Flash_CreateAppStatusCrc(o_pCrc)                                                                               \
	do {                                                                                                               \
...
void Flash_OperateMainFunction(void) {
	...
	if (true == bIsOperateFinshed) {
		...
		if ((gs_stFlashDownloadInfo.errorCode != true) &&
			((FLASH_ERASING == currentFlashJob) || (FLASH_PROGRAMMING == currentFlashJob) ||
...
tFlDownloadStepType Flash_GetCurDownloadStep(void) { return gs_stFlashDownloadInfo.eDownloadStep; }
...
/*save download data information, the API called by UDS request download
 * service*/
...
tFlshJobModle Flash_GetOperateFlashActiveJob(void) { return gs_stFlashDownloadInfo.eActiveJob; }
	...
	case DO_ERASING_FLASH:
		...
		if (totalSectors <= maxEraseSectors) {
			while (s_appFlashItem) {
				eraseFlashLen =
					s_pAppFlashMemoryInfo->xBlockEndLogicalAddr - s_pAppFlashMemoryInfo->xBlockStartLogicalAddr;

				/*fed watchdog*/
				WATCHDOG_HAL_Fed();

				sectorNo =
					FLASH_HAL_GetFlashLengthToSectors(s_pAppFlashMemoryInfo->xBlockStartLogicalAddr, eraseFlashLen);
			...
			while ((s_eraseSectorsCnt < totalSectors) && (0u != s_appFlashItem)) {
				...
				if ((eraseFlashStartAddr >= s_pAppFlashMemoryInfo->xBlockStartLogicalAddr) &&
					...
					/*calculate length*/
					eraseFlashLen = s_pAppFlashMemoryInfo->xBlockEndLogicalAddr - eraseFlashStartAddr;
				} else {
					s_result = false;
				...
				/*if erase max Erase sectors and have some sectors wait to
				 * erase, then request time from host*/
		...
		if ((false == *o_pbIsOperateFinsh) && (true == s_result) && (s_eraseSectorsCnt < totalSectors)) {
			...
			if (NULL_PTR != gs_stFlashDownloadInfo.pfRequestMoreTime) {
				gs_stFlashDownloadInfo.pfRequestMoreTime(gs_stFlashDownloadInfo.requestActiveJobUDSSerID,
														 RequetMoreTimeSuccessfulFromHost);
			}
		} else {
...
static uint8 Flash_Write(boolean *o_pbIsOperateFinsh) {
	...
	while (gs_stFlashDownloadInfo.receiveProgramDataLength >= PROGRAM_SIZE) {
		...
		if ((true == Flash_IsFlashEraseSuccessful()) && (true == Flash_IsFlashStructValid())) {
			WATCHDOG_HAL_Fed();

			/*write data in flash*/
			if (NULL_PTR != gs_stFlashDownloadInfo.stFlashOperateAPI.pfProgramData) {
				DisableAllInterrupts();
				result = gs_stFlashDownloadInfo.stFlashOperateAPI.pfProgramData(
					gs_stFlashDownloadInfo.startAddr,
					&gs_stFlashDownloadInfo.aProgramDataBuff[flashDataIndex * PROGRAM_SIZE], PROGRAM_SIZE);
				EnableAllInterrupts();
			} else {
				result = false;
			}
	...
	/*calculate if program data is align < 8 bytes, need to fill 0xFF to align 8
	 * bytes.*/
	if ((0u != gs_stFlashDownloadInfo.receiveProgramDataLength) && (true == result)) {
		...
		fsl_memset((void *)&gs_stFlashDownloadInfo.aProgramDataBuff[flashDataIndex * PROGRAM_SIZE +
...
/*get checksum step*/
static tChecksumStep Flash_GetChecksumStep(void) { return gs_eChecksumStep; }
...
static uint8 Flash_Checksum(boolean *o_pbIsOperateFinsh) {
	...
	/*
	 * flash driver data or APP data (flash driver is downloaded) can be
	 * calculate CRC
	 */
	if ((true == Flash_IsFlashDriverSoftwareData()) ||
	...
	if (true == *o_pbIsOperateFinsh) {
		...
		if (true == Flash_IsReceivedCRCValid()) {
			...
			FLS_DebugPrintf(LOG_LEVEL_ERROR, "CRC failed!\n");
...
static boolean Flash_DoChecksum(boolean *o_pbIsOperateFinsh) {
	...
	*o_pbIsOperateFinsh = isOperateFinsh;
	...
	if (true != isSuccessful) {
		FLS_DebugPrintf(LOG_LEVEL_ERROR, "Checksum job failed!\n");
...
void Flash_SavedReceivedCheckSumCrc(uint32 i_receivedCrc) { gs_stFlashDownloadInfo.receivedCRC = (tCrc)i_receivedCrc; }
...
uint32 Flash_GetResetHandlerAddr(void) { return gs_stAppFlashStatus.appStartAddr; }
...
static boolean Flash_IsFlashDriverSoftwareData(void) {
	...
	if ((gs_stFlashDownloadInfo.startAddr >= flashDriverStartAddr) &&
		((gs_stFlashDownloadInfo.startAddr + gs_stFlashDownloadInfo.length) < flashDriverEndAddr) && (true == result)) {
		result = true;
	} else {
		result = false;
	}
...
bool Flash_IsReadAppInfoFromFlashValid(void) {
	tCrc xCrc = 0u;
...
bool Flash_IsAppInFlashValid(void) {
	return Flash_IsFlashProgramSuccessful() && Flash_IsFlashEraseSuccessful() && Flash_IsFlashStructValid();
}
...
uint8 Flash_WriteFlashAppInfo(void) {
	...
	if (true == result) {
...
#else
			/*get app start address from flash. The address is the newest APP,
			 * because the APP info not write in flash, so the APP is old*/
			resetHandleAddr = appInfoStartAddr + resetHandleOffset;
#endif
			Flash_SaveAppResetHandlerAddr(*((uint32 *)resetHandleAddr), resetHandleLength);

			FLS_DebugPrintf(LOG_LEVEL_INFO, "APP type =%X, APP address=0x%lX\n", oldAppType, *((uint32 *)resetHandleAddr));
		...
		if (NULL_PTR != pAppStatusPtr) {
			...
			result = FLASH_HAL_WriteAPPInfoData(appInfoStartAddr, (uint8 *)pAppStatusPtr, sizeof(tAppFlashStatus));
#else
			if (NULL_PTR != gs_stFlashDownloadInfo.stFlashOperateAPI.pfProgramData) {
				result = gs_stFlashDownloadInfo.stFlashOperateAPI.pfProgramData(
					appInfoStartAddr, (uint8 *)pAppStatusPtr, sizeof(tAppFlashStatus));
			}
#endif
		} else {
			result = false;
