...
extern uint8 Flash_ProgramRegion(const uint32 i_addr, const uint8 *i_pDataBuf, const uint32 i_dataLen);

/*Is read application inforemation from flash valid?*/
extern bool Flash_IsReadAppInfoFromFlashValid(void);

/*Is application in flash valid? If valid return TRUE, else return FALSE.*/
extern bool Flash_IsAppInFlashValid(void);
...
extern void Flash_SetOperateFlashActiveJob(const tFlshJobModle i_activeJob, const tpfResponse i_pfActiveFinshedCallBack,
